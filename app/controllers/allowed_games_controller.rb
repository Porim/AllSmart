class AllowedGamesController < ApplicationController
  ...
  def destroy
    ...
    kid_games = @kids.allowed_games.where("created_at > ?", @allowed_game.created_at).order('created_at ASC').limit(1)
    if kid_games.empty?
      # if earlier created games exit, arrange them in descending 
      # order to find the the most recent one.
      @existing_games = @kids.allowed_games.where("created_at < ?", @allowed_game.created_at).order('created_at DESC').limit(1)
    else
    ...
    redirect_to kid_path(@kids, anchor: game_anchor) 
  end
  
  private
  
  def set_kid
    @kids = params[:user].present? ? Kid.find(params[:user][:kid_ids].select {|id| id unless id.empty? }) : Kid.find(params[:kid_id])
  end
end
